...
Coupled literal/length and distance swaps are now
...
| Route-local Huffman/header scratch reuse | A symbolized Mittens profile reduced top-stack allocator samples in the DeflOpt/order heap path from 209 to 78. Four warmed paired Default runs averaged 3.43 → 3.39 s user time, about 1.3%; elapsed time was about 2% lower but close to run noise. Equal-time Max runs retained the same 78,808-byte / 630,459-bit winner and used the saved work inside the fixed deadline | Reused and standalone heap builders match across every variant and several alphabet sizes; header, short-family, and full release tests preserve candidate order, exact bit accounting, and output |
