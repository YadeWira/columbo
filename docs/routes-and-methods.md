...
flowchart TD
    ...
    R10 -- "No" --> R11{"R11 · sweep improved + G0"}
    RESPONSE --> R11
    R11 -- "Yes" --> R1
    R11 -- "No" --> PICKMAX["Select byte/meaningful-bit winner<br/>or relaxed raw source"]
```

For stream owners admitted by G0t, G0 uses the primary phase deadline through M10d,
then the original full deadline from the terminal tree floors onward. R11
...
no mandatory
Default work, extra time or grace. The final Max incumbent receives the R1–R10 sequence and may repeat it
through R11 while G0 remains open.
