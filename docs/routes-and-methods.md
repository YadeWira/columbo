...
For compact stream owners, G0 uses the primary phase deadline through M10d,
...
| G0t · compact Max terminal share | Max under `Complete` or `CompleteThenBounded`, with nonzero allowance, source compressed and decoded size ≤128 KiB and ≤128 source blocks. These reuse the full R1–R5 work class. | Primary routes receive 4/5 of the original allowance, including elapsed parsing and floor work, with zero phase grace. Terminal tree floors and R1–R10 use the original full deadline and original grace. A primary yield is not a file timeout. Both phases grow with the allowance; no worker or extra time window is added. Larger streams and shared/established container children retain their earlier schedule. M10d may complete its coarse rescue after a primary yield only while the full soft deadline still permits terminal work. |
