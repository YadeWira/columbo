...
flowchart TD
    ...
    R9 -- "No" --> R10{"R10 · sweep improved + G0"}
    COUPLED --> R10
    R10 -- "Yes" --> R1
    R10 -- "No" --> PICKMAX["Select byte/meaningful-bit winner<br/>or relaxed raw source"]
```

For stream owners admitted by G0t, G0 uses the primary phase deadline through M10d,
then the original full deadline from the terminal tree floors onward. R10
...
no mandatory
Default work, extra time or grace. The final Max incumbent receives the R1–R9 sequence and may repeat it
through R10 while G0 remains open.
...
- Revisiting R1–R9 after later terminal improvements while skipping each
  method already tried on the unchanged generation. Only strict descent makes
  the nine score slots a valid identity check within this particular lineage.
